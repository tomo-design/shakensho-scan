...
(function () {
  const EXACT = {
    ...
    // ===== 設定 / クラウド =====
    "クラウド同期": "Cloud sync",
    "ログイン": "Log in", "管理者として会社を新規登録": "Register a company (as admin)", "メンバーとして会社に参加": "Join a company (as member)",
    ...
    "変更する": "Change", "（変更済・再変更は運営へ）": "(changed; contact us to change again)",
    "契約・プラン": "Plan & billing", "この端末": "This device", "解除": "Remove", "登録端末": "Registered devices",
    "➕ 追加端末（3台目〜・有料）": "➕ Add device (3rd+ · paid)",
    "2台目まで無料。3台目以降は「➕ 追加端末」で登録でき、追加分は月額/年額に自動で合算されます。": "Up to 2 devices are free. Add a 3rd+ via “➕ Add device”; extra devices are added to your monthly/yearly bill automatically.",
    "⛔ この端末は無料枠（2台）を超えています。": "⛔ This device exceeds the free limit (2 devices).",
    ...
    "⚠ この事業所IDは既に登録されています。メンバーとして参加してください。": "⚠ This company ID already exists. Please join as a member.",
    ...
    "メンバーとして会社に参加（承認待ちになります）": "Join a company as a member (pending approval)",
    ...
    "運営管理者": "Operator", "代表管理者": "Company admin", "メンバー": "Member",
    "メンバーがいません。": "No members.", "メンバーなし": "No members", "未ログイン": "Never logged in",
    "🔄 最新に更新 ": "🔄 Refresh ", "🛡️ 運営管理者ログイン": "🛡️ Operator login",
    "承認": "Approve", "却下": "Reject", "停止": "Suspend", "無効化": "Deactivate", "代表者に": "Make admin", "メンバーに": "Make member",
    ...
    // ===== 従業員登録の手順 =====
    "📋 メンバーの登録手順（はじめての方へ）": "📋 Member registration steps (first-time users)",
  ...
  const PH = [
    ...
    [/^登録 (.*)$/, "Registered $1"],
    [/^店舗コード: (.*)$/, "Store code: $1"],
    [/^📱 登録端末（(\d+)\/(\d+)台）(.*)$/, "📱 Registered devices ($1/$2)$3"],
    [/^ \/ (\d+)台（無料枠）$/, " / $1 (free)"],
    [/^ \/ (\d+)台（無料(\d+)＋追加(\d+)）$/, " / $1 ($2 free + $3 added)"],
    [/^最終利用: (.*)$/, "Last used: $1"],
    [/^役割: 代表管理者$/, "Role: Owner"],
    [/^役割: メンバー$/, "Role: Member"],
    [/^役割: 運営管理者$/, "Role: Operator"],
    [/^役割: (.*)$/, "Role: $1"],
