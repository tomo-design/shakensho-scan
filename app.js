...
function buildSpecPrompt() {
  return [
    ...
    "【締付トルクは網羅的に】その車両に存在する主要な締付トルクは省略せず必ず specs に含めること。最低でも次は該当があれば必ず載せる(値が不確かでも項目名を出し『（要確認）』を付ける。項目自体を省略しない): ホイールナット、アクスルシャフト(ハブ)ナット／ドライブシャフト(ハブ)ナット、ブレーキキャリパー取付、ドレンボルト(エンジン/ミッション/デフ)。特に『アクスルシャフト(ハブ)ナット締付トルク』は、リジッドアクスル・FR・4WD・トラック等ほぼ全車に存在するので、構造上明確に無い場合を除き必ず含めること。判断に迷う場合は省略せず『（要確認）』付きで載せる。",
