...
function renderRepairAnswer(box, obj, q) {
  ...
  if (order.length) {
    ...
    order.forEach(o => {
      ...
      let nm;
      if (sq) {
        // 部品名タップ → 折り畳みで楽天/Yahoo!/Amazonの検索リンクを開く(▾)
      ...
      if (sq) {
        ...
        const shops = document.createElement("div"); shops.className = "orderShops hidden";
        ...
        nm.addEventListener("click", () => {
          const nowHidden = shops.classList.toggle("hidden");
          nm.classList.toggle("open", !nowHidden);
        });
      }
    });
    // コピー/共有テキスト(品番なし)
