...
function buildRepairPrompt(q, hasMedia) {
  return [
    ...
    "【締付トルクは調べた実値のみ。推測値は出さない】torque欄は、この型式・エンジンの整備要領書・整備解説・分解整備事例をGoogle検索で実際に調べ、確認できた部位のみ『規定値(±公差があれば併記)』を具体数値で書く。安直な『(要確認)』の乱発は避け、まずは一次情報を粘り強く探すこと。ただし十分に探しても確かな一次情報が得られなかった部位は、推測値・目安値を数値で書いてはいけない。その部位は数値を出さず『未確認（整備要領書で確認）』と明示すること。特にホイールナット・ハブ/アクスル・ブレーキ・ステアリング・サスペンションなど保安・人命に関わる締結は、確実な出典が無い限り絶対に推定値を出さない(間違ったトルク値は事故につながる)。創作・他車種からの流用・『一般的な目安』での数値提示は禁止。",
...
$("btnKarteSave") && $("btnKarteSave").addEventListener("click", async () => {
  stopFieldMic();
  const work0 = $("kWork").value.trim();
  ...
  const entry = {
    ...
    date: sorted.date || "", odo: sorted.odo ? Number(sorted.odo) : null,
    work: sorted.work || "", parts: sorted.parts || "", cost: sorted.cost ? Number(sorted.cost) : null,
...
/* AIで整理(自動): 人それぞれの書き方(1欄に全部書く/略字/箇条書き等)をAIが判読し、
   作業・部品・費用・担当・日付・走行・メモの各項目に仕分けして整形する。
   保存時に自動で呼び出す。cur={date,odo,work,parts,cost,staff,note}(すべて文字列)を渡すと、
   仕分け済みの同形オブジェクトを返す。AIが使えない/失敗した時は cur をそのまま返す(保存は止めない)。 */
async function karteAiSortFields(cur) {
  const raw = [cur.work, cur.parts, cur.note].filter(Boolean).join("\n");
  // 仕分ける文字が無い(数値だけ等)なら、AIを呼ばずそのまま返す
  if (!raw) return Object.assign({}, cur);
  const prompt = [
  ...
  try {
    ...
    return {
      date: obj.date ? String(obj.date).trim() : cur.date,
      odo: (obj.odo != null && obj.odo !== "") ? String(obj.odo).replace(/[^\d]/g, "") : cur.odo,
      work: obj.work != null ? String(obj.work).trim() : cur.work,
      parts: obj.parts != null ? String(obj.parts).trim() : cur.parts,
      cost: (obj.cost != null && obj.cost !== "") ? String(obj.cost).replace(/[^\d]/g, "") : cur.cost,
      staff: obj.staff ? String(obj.staff).trim() : cur.staff,
      note: obj.note != null ? String(obj.note).trim() : cur.note,
    };
...
function viewDiagRecord(rec) {
  ...
  // 保存した診断の続きから相談できるよう、見解の下に「追加で相談」欄を置く(過去結果を文脈に続行)
  appendAiFollowup(body, rec.input || "", rec.aiText || "", { kind: "diag" });
...
function canonSpecKey(k) {
  ...
  // 変速機油(CVT/ATF/MT/トランスミッション)は表記ゆれが多く重複しやすい → 1項目に統合(デフ・エンジンは除外)
  if (/(ミッション|トランスミッション|変速機|cvt|atf|mtf|ギヤオイル|ギアオイル)/.test(s) && !/(デフ|デファレンシャル|エンジン)/.test(s)) return "ミッションオイル量";
...
function canonDisplayKey(k) {
  ...
  // 変速機油(CVT/ATF/MT)は表記ゆれで重複しやすいので1項目に統合(デフ・エンジンは除外)
  if (has(/(ミッション|トランスミッション|変速機|cvt|atf|mtf|ギヤオイル|ギアオイル)/) && !has(/(デフ|デファレンシャル|エンジン)/)) return "ミッションオイル量";
...
function buildSpecPrompt(known, missOnly) {
  ...
  const head = [
    ...
    "【必須項目は必ず1行ずつ出す】上記①〜⑨は、その車両に存在する限り必ずspecsに個別の行として含めること。特に②ミッションオイル量・③デフオイル量・⑤アクスルフランジ・⑥⑦前後ハブベアリングナットの出し忘れが多い。牽引車(トラクタ)・単車などの自走車ではこれらは通常存在するので省略しないこと。",
    "【被牽引車(トレーラー)の判定と扱い】この車両が被牽引車＝トレーラー(セミトレーラ・フルトレーラ・ポールトレーラ・ドリー、メーカー例: トレクス/日本トレクス/東邦車輌 等)の場合は、自走用の原動機・変速機・差動装置を一切持たない。判定材料: 原動機型式が空欄/『—』/記載なし、車体の形状が『セミトレーラ』『トレーラ』等、用途が被牽引。仮に車種DB一致やmodel推定でトラクタ名(例: UDクオン等)が付いていても、原動機型式が無いならトレーラーとして扱うこと。トレーラーと判断した場合: ①エンジンオイル量 ②ミッションオイル量 ③デフオイル量 と 推奨オイル粘度・クーラント量・エンジン型式の打刻位置・OBD検査 は構造上存在しないので一切出力しない(『該当なし』行も出さない)。代わりにトレーラーに実在する項目を出す: ホイールナット締付トルク、フロント/リアに相当する各アクスルのハブベアリングナット締付トルク、車軸(アクスル)まわりの締付、ブレーキ(エア/ドラム)関係の点検・調整値、エアタンク/エア配管、タイヤ空気圧、キングピン(第五輪)まわりの点検、車台番号の打刻位置。トレーラーであるにもかかわらずトラクタ用のエンジン諸元を出すことは重大な誤りなので絶対に避けること。",
    ...
    "『オイルエレメント』『オイル交換目安』『スパークプラグ』『バッテリーサイズ』の項目は出力しないこと(必須9項目とは別に、確証がある場合のみ他項目を足してよいが、確証が無いなら足さない)。",
    "【追加項目に要確認を付けない】必須9項目以外の項目は、確かな一次情報で数値が確定できる場合のみ出す。『（要確認）』を付けるくらいなら、その項目自体を出力しないこと(要確認だらけにしない)。要確認が許されるのは必須9項目だけ。",
    ...
    "【変速機油は必ず1行だけ】CVTフルード・ATF・ミッションオイル・トランスミッションオイル・CVTフルード容量 等は、すべて同じ『変速機の油』を指す表記ゆれである。これらを複数行に分けて出さず、必ず『ミッションオイル量』の1行に統合する(油種名＋交換時量／全容量を1つにまとめる)。デフオイルは別物なので統合しない。",
...
const SUPPORT_KB = [
  ...
  "【画面】下タブ=スキャン/履歴/DB編集/設定。車両を開くと上部に 車両/メンテ/診断/修理/カルテ。PC・タブレット横向き(画面幅1024px以上)では下タブが画面左のサイドメニューになり、診断・修理は左に入力/右にメカ君の回答、メンテは左に諸元/右に定番故障・リコール、カルテは記録を書きながら過去の記録を横に見られる2カラム表示。タブレット縦向きは画面を広く使い、履歴・設定は2列。",
