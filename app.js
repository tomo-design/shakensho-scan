...
const GEMINI_MODELS = {
  ...
  flash: ["gemini-3.6-flash", "gemini-flash-latest", "gemini-flash-lite-latest", "gemini-2.0-flash"],
...
const GEMINI_MEDIA_MODELS = {
  ...
  flash: ["gemini-3.6-flash", "gemini-flash-latest", "gemini-2.0-flash"],
  pro: ["gemini-3.1-pro-preview", "gemini-pro-latest", "gemini-3.6-flash", "gemini-flash-latest", "gemini-2.0-flash"]
};
...
const SUPPORT_KB = [
  ...
  "",
  "【概要】整備士向けアプリ。車検証をスキャンして車両を識別し、メンテナンス諸元・AI故障診断・修理手順・整備カルテを現場で使える。データは端末内に保存。契約店舗は社内の全端末で自動共有。個人版(Google Play)は7日無料→月額¥500。",
  ...
  "【会社共有・参加】契約店舗は車両・カルテを全端末で共有。従業員は代表管理者の承認で参加(設定→クラウド同期→『会社に参加』でメール・パスワード・事業所IDを入力→承認待ち)。1人2端末まで。代表管理者は複数人指名できる(メンバー管理→『代表者に』)。",
  ...
  "【料金プラン(法人)】3段階。N/A=裏取り検索なし(標準AI・全機能可)。ターボ=AI Pro＋裏取り検索(月500回)。ツインターボ=AI Pro＋裏取り検索(無制限・検索を使えるのは席数まで=標準3席、4席目〜+¥3,000/席)。『裏取り検索』はGoogle検索で実データを確認し精度を上げる機能(診断・修理で有効)。現在の契約特典は設定→『契約・プラン』にグレード別に表示。プラン変更・席追加は同画面(代表管理者)または運営。支払いは月額/年契約。",
  "【個人版(Google Play)】ダウンロード無料、最初の7日間は全機能無料、8日目以降は月額¥500(アプリ内サブスク)。ログイン不要で使える。",
  "【困った時】『AIが混み合っています』は少し時間をおいて再試行。諸元が遅い/出ない時は『最新に更新』や個別🔄、契約店舗はプラン有効期限を確認。データは端末を削除すると消えるので大切な記録は控えを。アプリが古い時は設定下部『アプリを最新に更新』(個人版アプリはストア更新)。",
