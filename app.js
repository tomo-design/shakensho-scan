...
function buildMediaDiagPrompt() {
  const lines = [
    ...
    "【統合診断】写真に複数のDTC(ダイアグコード)や複数の症状が写っている場合は、1つずつ別々に原因を挙げず、全体を『1つの故障像』としてまとめて解釈し、それらを一括で説明できる根本原因を最優先で特定すること。表面的なコード名に引っ張られず、原因のコードと結果(二次的)のコードを見分ける。第1位の候補はできるだけ多くのDTC・症状を説明できる根本原因にする。",
