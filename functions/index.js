...
async function issueContractAccount(db, info) {
  ...
  uid = u.uid;
  const edition = info.edition === "personal" ? "personal" : "works";
  // Pocket(個人版)はログインID＝申込メールアドレス。法人はテナント発行のログインID。
  let loginId = edition === "personal" ? email : pr.loginId;
  ...
  if (edition === "personal") {
    ...
    const bodyP = (name ? name + " 様" : "お客様") + "\n\n" +
      ...
      "下記の情報で、すぐにご利用いただけます。\n\n" +
      "▼ログインID\n" + loginId + "\n\n" +
      "▼初期パスワード\n" + pw + "\n（安全のため、初回ログイン後に『設定タブ → 🔑パスワードを変更』で任意のパスワードへ変更してください）\n\n" +
...
2) 返信メールの本文を、CS担当として丁寧かつ簡潔に作成する。製品KBの事実のみを根拠にし、金額はKB記載の範囲で正確に。答えられない点は「担当より折り返す」と案内。誇張・虚偽は書かない。CTAは押しつけない。
   ★人間味(重要): いかにもAIが書いたと分かる機械的・テンプレ的な文章にしない。実在の担当者が自分の言葉で書いた自然なメールにする。定型フレーズの乱発・「〜させていただきます」の多用・絵文字/記号の過剰装飾・見出しの詰め込みを避け、相手の問い合わせ内容に具体的に触れた一言を自然に添える。堅すぎず、誠実で温度感のある文面に。AIである旨や自動生成である旨のメタ発言は絶対にしない。
...
【キャッチコピー／核メッセージ(営業メール・DM・チラシで必ず織り込む)】「AIと共に働く時代へ。人手不足を補い、若手が育つ現場をつくる。」（＝AIと人が共存し、人手不足の解消と人材育成につながる、という思想）
・これは"これからの整備現場はこう変わる"という時代のビジョン。営業文の軸として、書き出し(つかみ)か締めのどちらかで必ず一度は伝える。
・一字一句同じでなくてよい。相手(整備工場/運送/ディーラー)や文脈に合わせてニュアンスを変えて言い換えてよい（例:「AIに置き換えられるのではなく、AIと組んで人が伸びる時代です」「調べ物と記録はAIに任せ、人は判断と技術に集中。それが人材育成にもつながります」等）。
・"人を置き換える"のではなく"人の力を引き出す・共存する"というトーンを守る（脅しや煽りにしない）。
...
【主な機能】※先頭の「故障診断の一括検索」＋「修理手順の提示」がメカノAI最大の売り。営業では必ずこの2つを主役に据える。
・★故障診断の一括検索（最大の売り）: 故障コード(DTC)や「エンジンがかからない/警告灯が点いた」等の症状を入力するだけで、考えられる原因・点検すべき箇所・対処法をAIが一括で提示。これまで整備書・過去事例・ネットを何本も横断して調べていた"原因の切り分け"が、一度の検索でまとまって出る。裏取り検索にも対応。若手やベテラン不在時でも診断の当たりを素早くつけられる（＝人手不足・若手育成にも直結）。
・★修理手順の提示（もう一つの主役）: 原因の特定で終わらず、その先の"どう直すか"＝作業手順・交換部品・注意点・締付トルク等の実作業ステップまでAIが提示。診断→修理までを一気通貫でサポートするので、経験の浅いスタッフでも段取りに迷わず作業に入れる。手順は自社のやり方に直して保存でき、使うほど自社標準の手順書として蓄積する。
...
【プラン変更と日割り精算(よくある質問)】設定タブ→プランを変更する、で NA↔ターボ↔ツインターボ をいつでも変更できる。★変更しても契約は1本のまま(新規契約が二重に増えない)。①無料トライアル中(契約7日以内)に変えた場合＝まだ¥0なので請求は起きず、トライアル終了後は"変更後プラン"の料金で初回請求。②有料期間に入ってから変えた場合＝Stripeが「元プランの残り日数分」と「新プランの残り日数分」を日割りで自動計算し、その差額を次回の請求書にまとめて計上(その場で即請求されるわけではない)。アップグレード(例:NA→ターボ)は差額が加算、ダウングレード(例:ツインターボ→NA)は使わなかった分がクレジット(次回請求から自動で差し引き)になる。支払い間隔(月額↔年額)の変更も同じく日割りで次回請求に反映。
【支払い方法(よくある質問)】お支払いは請求書リンクから、カード払い（分割払い可）または銀行振込に対応。コンビニ払い・PayPayは現在準備中(未対応)。請求書は各サイクルで自動的に契約者のメールへ送付され、メール内のリンク(請求書ページ)から支払える。★銀行振込の場合＝請求書ページを開くと専用の振込先口座(Stripeが発行する仮想口座)と振込金額が表示され、そこへ振り込むと自動で入金照合・支払い完了になる(手作業の消込は不要)。カード払いの人も同じ請求書ページでその場でカード入力して支払える。支払い期限は請求書発行から14日。無料トライアル中(最初の7日)は¥0のため請求書は送られず、8日目の初回請求から送付が始まる。
【決済・セキュリティ(よくある質問・安心材料)】決済処理は世界標準の決済会社Stripe(ストライプ)が行い、通信は暗号化(SSL/TLS)される。★カード番号や振込情報はStripe側で厳重に管理され、メカノAI(当社)のサーバーやアプリにカード番号が保存されることは一切ない(Stripeは国際的なカード業界セキュリティ基準PCI DSSに準拠)。請求書や振込先口座は必ず正規のStripeドメイン(buy.stripe.com / 請求書URL)で表示されるので、そこ以外で口座やカードを入力しないよう案内してよい。整備データ(車両情報・カルテ等)は決済情報とは別に管理され、決済で外部に渡ることはない。特定商取引法に基づく表記・プライバシーポリシーも公開済み(https://mechanoai-cablueie.com/tokushoho.html ／ /privacy.html)。
【解約(よくある質問)】設定タブ→プランを変更する→「解約する」からいつでも解約可。解約後も、契約済み期間の満了までは利用できる。
...
【強み】故障診断の一括検索で"原因の切り分け"が一発、さらに"どう直すか"の修理手順まで提示。診断→修理を一気通貫。現場の紙作業・調べ物を削減。車検証から一気通貫。最新Geminiで高精度。整備士目線のUI。`;
...
【決済・セキュリティ(よくある質問・安心材料)】月額¥500の決済は世界標準の決済会社Stripe(ストライプ)が処理し、通信は暗号化(SSL/TLS)される。カード番号はStripe側で厳重に管理され、メカノAIのサーバーやアプリにカード番号が保存されることは一切ない(StripeはPCI DSS準拠)。カード入力は必ず正規のStripe画面(checkout.stripe.com等)で行われる。整備記録は端末内に保存され外部送信しないため、決済とデータは完全に別管理。解約は設定タブからいつでも可能で、契約済み期間の満了まで利用できる。
...
【核メッセージ（訴求の中心・重要）】価値は複数あり、特定の機能に偏らせない。次を"横並びの主役"として扱う: ①AI故障診断(症状の問診やダイアグコードDTCから原因を切り分け) ②修理のやり方(部品の外し方・交換手順・必要な工具SST・締付トルク・所要時間) ③メンテナンス諸元の即表示(締付トルク・オイル粘度/量など) ④整備カルテ(端末内保存) ⑤日本語/英語対応。共通の芯は「ベテランに聞けなくても、一人で作業を前に進められる」。文面ごとにテーマに合う切り口を選び、毎回トルク/オイルの話に偏らないこと(診断・修理のやり方も同等に前面に出す)。ワンコイン(¥500/月)で全機能、まずは7日間無料。
...
const SALES_STAFF = {
  ...
  writer: {
    name: "セールスライター 文乃",
    sys: "あなたはメカノAI販売会社のセールスライター。整備工場の経営者・工場長に響く提案メール/DM/チラシ文面を書く。専門用語を使いすぎず、導入メリット(時間短縮・ミス削減・若手教育)を具体的に。CTA(無料デモ・問い合わせ)で締める。過度な誇張・虚偽は書かない。★どの文面でも核メッセージ『AIと共に働く時代へ。人手不足を補い、若手が育つ現場をつくる』を、書き出しか締めで必ず一度は伝える(ニュアンスは言い換え可、『人を置き換えるのではなく人の力を引き出す』トーンを守る)。",
  },
  marke: {
    name: "マーケ担当 舞",
...
exports.salesRoom = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).region(REGION).https.onRequest(async (req, res) => {
  ...
  const angleRule = product === "pocket"
    ? "訴求の主軸: 個人の整備士に向けて、特定の機能に偏らせない。次を横並びの価値として扱う——①AI故障診断(症状の問診・ダイアグコードDTCから原因の切り分け) ②修理のやり方(部品の外し方・交換手順・工具SST・締付トルク・所要時間) ③メンテナンス諸元の即表示(締付トルク・オイル粘度/量など) ④整備カルテ ⑤日本語/英語。共通の芯は『ベテランに聞けなくても一人で作業を前に進められる』。その文面/投稿のテーマに合う切り口を選び、毎回トルク・オイルの話に偏らせない(診断・修理のやり方も同等に主役として扱う)。ワンコイン(¥500/月)・7日間無料・自分の無料APIキー(クレカ不要)も添える。誇張はしない。"
    : "訴求の主軸: 特に別の指定がない限り、まず『故障診断の一括検索』＋『修理手順の提示』の2つをメカノAI最大の売りとして前面に出す（DTCや症状を入れるだけで原因・点検箇所・対処を一度にまとめて提示し、さらに『どう直すか』の作業手順・交換部品・注意点・締付トルクまで出る＝診断から修理まで一気通貫）。その上で、これが『整備士の人手不足の解消』と『若手を即戦力化できる（ベテラン不在でも診断の当たりをつけて手順に迷わず作業に入れる）』にどう効くかへ着地させる。冒頭で現場の課題（診断・調べ物の負担）に触れ、解決策としてこの『診断→修理の一括サポート』を提示する流れにする。諸元・トルクの即表示はあくまで補助的な強みとして添える程度に留め、主役は故障診断一括検索と修理手順。ただし誇張や虚偽は書かない。";
  // ★製品(Works/Pocket)の機能・概念を絶対に混同させないための最重要ガード。混在はトラブルの元。
  const mixRule = product === "pocket"
    ? "★製品混同の絶対禁止(最重要): 今回はPocket(個人向け)。Pocketは『整備士ひとりが自分のスマホ1台・端末内で完結して使う』アプリ。したがって『社内共有／チーム共有／店舗共有／複数人・複数端末での共有』『入庫管理ボード』『メンバー管理・端末管理』『会社契約・代表管理者』『プラン(NA/ターボ/ツインターボ)』などは一切ない —— これらは全て法人版Works専用の機能。これらの語・概念を文章に絶対に入れない。データは端末内のみで、『社内で共有される』とは決して書かない。『使うほど育つ／自分仕様の資産になる』を書く場合も『自分専用の相棒として育ち、自分の記録が蓄積する』に限定し、共有・チーム・社内には触れない。宛先は『あなた(個人の整備士)』であり、会社宛・店舗宛の口調にしない。★リンク・導線も個人向けのみ: 法人向けの 法人LP(biz.html)・法人申込フォーム(apply.html)・法人サービス紹介資料(shiryou.html)・会社署名(担当:中江…) は絶対に載せない。『サービス詳細・資料はこちら』のようにアプリ紹介の資料URLを載せる場合は、必ず Pocketのご案内資料 https://mechanoai-cablueie.com/haifu-personal.html を使う(Worksのbiz.htmlは絶対に使わない)。案内してよいURLは次の4つのみ: ①Pocketのはじめ方=mechanoai-cablueie.com のログイン画面『Pocket＞7日間無料ではじめる』 ②体験デモ=mechanoai-cablueie.com/?demo=1 ③Pocketご案内資料=mechanoai-cablueie.com/haifu-personal.html ④Pocketのよくある質問=mechanoai-cablueie.com/faq.html?v=personal。★もう一方の版の『存在』だけは伝えてよい: 主役はPocketのまま、文末に1文だけ『店舗・チームで共有して使いたい場合は法人版の MECHANO-AI Works もあります』程度でさりげなく案内する(Worksの機能詳細は書かない・Pocketの説明にWorks機能を混ぜない)。短すぎる媒体(短文投稿・返信)では省略してよい。"
    : "★製品混同の絶対禁止(最重要): 今回はWorks(法人向け)。個人版Pocket特有の文脈(『個人1台・端末内のみで完結』『会社で共有しない』等)と混同しない。Worksは店舗・チームで車両データ/整備記録/入庫管理を共有できるのが価値。★もう一方の版の『存在』だけは伝えてよい: 主役はWorksのまま、文末に1文だけ『整備士が個人のスマホ1台で使いたい場合は個人版の MECHANO-AI Pocket もあります』程度でさりげなく案内する(Pocketの機能詳細は書かない・Worksの説明にPocket機能を混ぜない)。短すぎる媒体(短文投稿・返信)では省略してよい。";
...
人間味ルール(重要): いかにもAIが自動生成したと分かる文章にしない。実在の営業担当が自分の言葉で書いた自然な文章にすること。具体的には—
・機械的な定型フレーズや過度に完璧・均質な言い回しを避け、口語のリズムや軽い言い回しの揺れを自然に混ぜる。
・「〜させていただきます」の乱発、絵文字の多用、記号での過剰装飾、箇条書きの詰め込みすぎ、テンプレ感の強い見出し（【ご提案】等の乱用）を避ける。
・相手や状況に触れた一言（現場をねぎらう/季節/前回のやりとり等）を1つだけ自然に入れて、手書き感を出す。ただし取ってつけた感やわざとらしさは禁物。
...
exports.bizInquiry = functions.region(REGION).https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (email) {
      ...
      if (intent === "contract" || isPocket) {
        // 契約希望(法人)/個人版(Pocket)申込 → テナント＋7日トライアル＋Authユーザー＋初期パスワードを
        // 自動発行し、ログインID・初期パスワード入りの案内メールを自動返送(運営の手作業ゼロ)。
        // PocketはログインID＝申込メールアドレス／edition=personal(Works不可)で発行する。
        // ※Stripe未設定でも provisionContract がトライアルで代替するため、必ずアカウントを発行する。
        try {
          const acc = await issueContractAccount(db, {
            company, name, email, plan,
            edition: isPocket ? "personal" : "works",
          });
          provisioned = true;
          replySubjectSent = acc.subject; replyBodySent = acc.body;
          sendMail(email, acc.subject, acc.body, replyAddr()).catch(() => {});
        } catch (e) {
          console.error((isPocket ? "Pocket" : "契約") + "自動発行エラー", e);
          // 既存アカウント等で発行できなかった場合は、下のフォールバック(受付メール)に回る。
        }
      }

      if (!provisioned && isPocket) {
        // 個人版(Pocket)で自動発行できなかった場合(既に登録済みメール等) → 受付のみ自動返信。
        const subject = "【メカノAI Pocket】お申し込みありがとうございます（自動返信）";
        const ack =
          "この度はメカノAI Pocket（個人版）へお申し込みいただきありがとうございます。\n\n" +
          "ご入力のメールアドレス（" + email + "）は、既にアカウントが登録されています。\n" +
          "お手数ですが、アプリの「Pocket ＞ ログイン」から既存のID・パスワードでログインしてください。\n\n" +
          "パスワードをお忘れの場合は、本メールにご返信いただければ担当が再設定のご案内をいたします。\n\n" +
    ...
    try {
      ...
      if (opTo) {
        const kindLabel = intent === "pocket" ? "個人版(Pocket)申込" : (intent === "contract" ? "契約手続きの希望" : "資料・デモ希望");
        const opSubject = (isPocket ? ("【Pocket申込" + (provisioned ? "・自動発行済み" : "・要対応") + "】") : ("【問い合わせ" + (intent === "contract" ? "・契約希望" : "") + "】")) + (company || "（会社名なし）") + " / " + (name || "");
        const opBody =
          (isPocket ? ("アプリのログイン画面(Pocket)から個人版の利用申込が届きました。" +
                       (provisioned ? "Pocket専用アカウント(ID＝申込メール／edition=personal)を自動発行し、認証情報を申込者へ送信済みです。\n\n"
                                    : "既存メール等で自動発行できませんでした。営業コンソールからご対応ください。\n\n"))
      ...
      if (uniqTokens.length) {
        await admin.messaging().sendEachForMulticast({
          ...
          notification: {
            ...
            body: intent === "pocket"
              ? (email + " から個人版(Pocket)の申込。" + (provisioned ? "専用アカウントを自動発行しました。" : "既存メール等で自動発行できず要対応。"))
              : (company + " / " + name + " さんから" +
                (intent === "contract"
